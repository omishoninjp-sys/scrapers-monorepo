...
def get_harada_duplicate_groups():
    products = []
    url = shopify_api_url("products.json?limit=250&vendor=Gateau+Festa+Harada&fields=id,title,variants,created_at,image")
    while url:
...
def run_dedup():
    ...
    try:
        products, duplicates = get_harada_duplicate_groups()
...
<script>
let pollTimer=null;
async function startScan(){document.getElementById("scanBtn").disabled=true;document.getElementById("deleteBtn").disabled=true;document.getElementById("statusText").textContent="掃描中...";document.getElementById("statsSection").style.display="none";document.getElementById("results").innerHTML="";try{const r=await fetch("/api/scan-duplicates");const d=await r.json();if(d.error){alert(d.error);return}document.getElementById("statScanned").textContent=d.total;document.getElementById("statGroups").textContent=d.duplicate_groups;document.getElementById("statToDelete").textContent=d.to_delete;document.getElementById("statDeleted").textContent="-";document.getElementById("statsSection").style.display="flex";document.getElementById("statusText").textContent=d.to_delete===0?"✓ 沒有重複商品":"";renderGroups(d.duplicates.slice(0,50));if(d.duplicates.length>50)document.getElementById("results").innerHTML+="<p style=\\"color:#888;text-align:center;font-size:13px\\">顯示前 50 組，共 "+d.duplicates.length+" 組</p>";document.getElementById("deleteBtn").disabled=d.to_delete===0}catch(e){alert(e.message)}finally{document.getElementById("scanBtn").disabled=false}}
function renderGroups(groups){document.getElementById("results").innerHTML=groups.map(g=>{const k=g.keep;let html="<div class=\\"group\\"><div class=\\"group-header\\">SKU: "+g.sku+"（共 "+(g.delete.length+1)+" 個，刪除 "+g.delete.length+" 個）</div>";html+="<div class=\\"product-row keep\\">"+(k.image?"<img src=\\""+k.image+"\\">":"<div class=\\"no-img\\">無圖</div>")+"<div class=\\"info\\"><div class=\\"title\\">"+k.title+" <span class=\\"badge badge-keep\\">保留</span></div><div>"+k.created_at.substring(0,10)+"</div></div></div>";g.delete.forEach(p=>{html+="<div class=\\"product-row\\">"+(p.image?"<img src=\\""+p.image+"\\">":"<div class=\\"no-img\\">無圖</div>")+"<div class=\\"info\\"><div class=\\"title\\">"+p.title+" <span class=\\"badge badge-del\\">刪除</span></div><div>"+p.created_at.substring(0,10)+"</div></div></div>"});return html+"</div>"}).join("")}
async function startDelete(){if(!confirm("確定刪除全部重複商品？此操作不可復原。"))return;document.getElementById("deleteBtn").disabled=true;document.getElementById("scanBtn").disabled=true;document.getElementById("progressBar").style.display="block";document.getElementById("progressFill").style.width="0%";document.getElementById("statusText").textContent="刪除中...";try{const r=await fetch("/api/delete-duplicates",{method:"POST"});const d=await r.json();if(d.error){alert(d.error);document.getElementById("scanBtn").disabled=false;return}pollTimer=setInterval(pollDeleteStatus,1500)}catch(e){alert(e.message);document.getElementById("scanBtn").disabled=false}}
async function pollDeleteStatus(){try{const r=await fetch("/api/dedup-status");const d=await r.json();const pct=d.to_delete>0?Math.round(d.deleted/d.to_delete*100):0;document.getElementById("progressFill").style.width=pct+"%";document.getElementById("statDeleted").textContent=d.deleted;document.getElementById("statusText").textContent="刪除中 "+d.deleted+"/"+d.to_delete+"，錯誤 "+d.errors.length;if(!d.running&&d.done){clearInterval(pollTimer);document.getElementById("progressFill").style.width="100%";document.getElementById("statusText").textContent="✓ 完成！刪除 "+d.deleted+" 個，錯誤 "+d.errors.length+" 個";document.getElementById("scanBtn").disabled=false}}catch(e){console.error(e)}}
</script>
</body></html>'''
...
def run_update_shipping():
    ...
    try:
        ...
        pids = []
        url = shopify_api_url("products.json?limit=250&vendor=Gateau Festa Harada&fields=id,body_html")
        while url:
            r = requests.get(url, headers=get_shopify_headers())
...
<body>
    ...
    <div class="card">
        ...
        <a href="/japanese-scan" class="btn btn-success">🇯🇵 日文掃描</a> <a href="/dedup-scan" class="btn" style="background:#e74c3c">🗑️ 重複清理</a> <button class="btn" style="background:#2ecc71" onclick="updateShipping()">📦 更新運費說明</button>
    </div>
    <div class="card">
        <h3>開始爬取</h3>
