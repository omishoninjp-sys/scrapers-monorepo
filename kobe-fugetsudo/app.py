...
def run_update_shipping():
    ...
    try:
        ...
        pids = []
        url = shopify_api_url("products.json?limit=250&vendor=神戶風月堂&fields=id,body_html")
        while url:
            r = requests.get(url, headers=get_shopify_headers())
            if r.status_code != 200:
