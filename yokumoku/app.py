"""
YOKUMOKU 商品爬蟲 + Shopify 上架工具 v2.1
v2.1: 翻譯保護機制、日文商品掃描、翻譯驗證重試、環境變數、Docker/Zeabur 部署
功能：
1. 爬取 yokumoku.jp 所有商品（排除冷凍）
2. 過濾無庫存商品
3. 計算材積重量 vs 實際重量，取大值
4. 上架到 Shopify（不重複上架）
5. 原價寫入成本價（Cost）
6. OpenAI 翻譯成繁體中文 + 翻譯驗證
7. 【v2.1】翻譯保護機制 - 翻譯失敗不上架、預檢、連續失敗自動停止
8. 【v2.1】日文商品掃描 - 找出並修復未翻譯的商品
9. 【v2.1】翻譯驗證 - 翻譯後仍含日文自動重試
"""
...
scrape_status = {
    ...
    "skipped_low_price": 0, "filtered_by_price": 0, "deleted": 0,
...
def load_shopify_token():
    """載入 Shopify Token（優先環境變數，備用 JSON 檔）"""
...
{{"title":"翻譯後的商品名稱（前面加上 YOKUMOKU）","description":"翻譯後的商品說明（HTML）","page_title":"SEO標題50-60字","meta_description":"SEO描述100字內"}}

規則：1.YOKUMOKU高級洋菓子 2.開頭「YOKUMOKU」3.シガール→雪茄蛋捲（非香菸蛋捲）4.禁日文 5.只回傳JSON"""
    if retry:
        prompt += "\n\n【嚴重警告】上次翻譯結果仍然包含日文字元（平假名/片假名）！這次你必須：\n1. 將所有日文平假名、片假名完全翻譯成繁體中文\n2. シガール→雪茄蛋捲、サンクデリス→五味精選、ビエ→薄餅、ショコラ→巧克力、ラングドシャー→貓舌餅乾、オレ→牛奶、詰合せ→綜合禮盒\n3. 絕對不可以出現任何 ひらがな 或 カタカナ\n4. 商品名中的日文必須全部意譯成中文"
...
def publish_to_all_channels(pid):
    ...
    requests.post(gu, headers=hd, json={'query': mut, 'variables': {
        "id": f"gid://shopify/Product/{pid}", "input": [{"publicationId": p['id']} for p in uq]}})
    return True
...
def parse_size_weight(text):
    ...
    if wm:
        ...
        weight_kg = float(gm.group(1)) / 1000

    final = 0
    if dimension and weight_kg:
        final = max(dimension.get('volume_weight', 0), weight_kg)
    elif dimension:
        final = dimension.get('volume_weight', 0)
    elif weight_kg:
        final = weight_kg
...
def scrape_product_detail(url):
    """使用 Playwright 爬取單一商品詳細資訊"""
    product = {
        'url': url, 'title': '', 'subtitle': '', 'price': 0, 'description': '',
        'size_weight_text': '', 'weight': 0, 'images': [], 'in_stock': True,
        'is_frozen': False, 'sku': ''
    }
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                page.wait_for_selector('.p-details', timeout=10000)
            except:
                pass
            time.sleep(5)
            ...
            for sel in ['.p-price__price', '.p-price', '[class*="price"]', '.price']:
                ...
                if el:
                    ...
                    if pm:
                        product['price'] = int(pm.group(1).replace(',', ''))
                        break
            ...
            for sel in ['.p-details__description', '.description', '[class*="description"]']:
                el = page.query_selector(sel)
                if el:
                    product['description'] = el.inner_html()
                    break
            ...
            if not product['weight']:
                ...
                if wm:
                    product['weight'] = round(float(wm.group(1).replace(',', '')) / 1000, 2)
            ...
            # 圖片
            images = []
            skip_patterns = ['dummy_product_thumbnail', 'play_button', 'details/caution/',
                             'about_clack', 'about_shopper', 'data:image/png;base64']
            ...
            if tc:
                for img in tc.query_selector_all('img'):
                    ...
                    if not src or any(pat in src for pat in skip_patterns):
                        continue
...
def upload_to_shopify(product, collection_id=None):
    """上傳商品到 Shopify（含翻譯保護 + 翻譯驗證）"""
    original_title = product['title']
    if product.get('subtitle'):
        original_title = f"{product['title']} - {product['subtitle']}"
    ...
    if is_japanese_text(translated['title']):
        ...
        retry_result = translate_with_chatgpt(
            original_title, product.get('description', ''),
            retry=True
        )
    ...
    sp = {'product': {
        ...
        'variants': [{'sku': product['sku'], 'price': f"{selling_price:.2f}", 'weight': weight,
                      'weight_unit': 'kg', 'inventory_management': None, 'inventory_policy': 'continue',
...
<h1>🍪 YOKUMOKU 爬蟲工具 <small style="font-size:14px;color:#999">v2.1</small></h1>
...
<p style="color:#666;font-size:14px">※ 排除冷凍商品 | &lt;¥__MIN_COST__ 跳過 | <b style="color:#e74c3c">翻譯保護</b> 連續失敗 __MAX_FAIL__ 次停止<br>⚠️ 使用 Playwright 無頭瀏覽器，商品列表載入需要較長時間</p>
...
def cron_trigger():
    global scrape_status
    if scrape_status['running']:
        return jsonify({'success': False, 'error': '爬取正在進行中'}), 409
    if not load_shopify_token():
        return jsonify({'success': False, 'error': '未設定環境變數'}), 500
    test = translate_with_chatgpt("テスト商品", "テスト説明")
    if not test['success']:
...
if __name__ == '__main__':
    print("=" * 50)
    print("YOKUMOKU 爬蟲工具 v2.1")
    print("新增: 翻譯保護、日文掃描、翻譯驗證")
    print("=" * 50)
